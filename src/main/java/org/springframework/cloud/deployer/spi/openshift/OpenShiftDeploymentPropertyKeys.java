...
public interface OpenShiftDeploymentPropertyKeys {
	...
	/**
	 * An optional comma separated list of labels to add to the DeploymentConfig. This
	 * could be useful for grouping different standalone/stream/tasks into common
	 * view/application group etc.
	...
	/**
	 * Create a NodePort instead of a Route. Either "true" or a number at deployment time.
	 * The value "true" will choose a random port. If a number is given it must be in the
	 * range that is configured for the cluster (service-node-port-range, default is
	 * 30000-32767).
	 */
