 ...
 */
public class WatchingBuildConfigFactory implements ObjectFactory<BuildConfig> {

	private static Logger logger = LoggerFactory
			.getLogger(WatchingBuildConfigFactory.class);
	...
	public WatchingBuildConfigFactory(OpenShiftClient client,
			BuildConfigFactory buildConfigFactory, OnCompletedCallback<Build> callback) {
		this.client = client;
	...
	public BuildConfig addObject(AppDeploymentRequest request, String appId) {
		return buildConfigFactory.addObject(request, appId);
	}

	@Override
	public void applyObject(AppDeploymentRequest request, String appId) {
		buildConfigFactory.applyObject(request, appId);
